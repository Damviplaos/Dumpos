...
export async function getProfile(userId: string): Promise<Profile | null> {
  ...
  // โหลด permission_roles ทั้งหมด (many-to-many)
  const { data: permRoleRows } = await supabase
    .from('user_permission_roles')
    .select('role:roles(*)')
    .eq('user_id', userId);

  const permission_roles = (permRoleRows ?? [])
    .map((r: any) => r.role)
    .filter(Boolean) as import('@/types/types').Role[];

  return { ...data, permission_roles };
}
...
/** รวม permissions จาก custom_role + permission_roles ทั้งหมด (union) */
function resolvePermissions(profile: Profile | null): Permissions {
  ...
  // รวม permissions จาก permission_roles ทั้งหมด (union)
  const merged: Permissions = {};
  if (profile.permission_roles && profile.permission_roles.length > 0) {
    for (const r of profile.permission_roles) {
      if (r.permissions) {
        for (const k of allKeys) {
          if (r.permissions[k]) merged[k] = true;
        }
      }
    }
    return merged;
  ...
  // fallback: single custom_role (backward compat)
